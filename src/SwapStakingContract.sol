    ...
    ReentrancyGuardUpgradeSafe
{
    ...
    bytes32 private constant REWARDS_DISTRIBUTOR_ROLE =
    ...
    function migrateSwap(address _address, address _swapHolder) public {
        require(
            _address != address(0),
        ...
        require(
            ...
            "[Validation] The caller must have owner role to migrate"
        );
        require(!hasMigrated);
        uint256 swapBalance = token.balanceOf(address(this));
        hasMigrated = true;
        token.transfer(address(0), swapBalance);
        token = IERC20(_address);
        token.transferFrom(_swapHolder, address(this), swapBalance);
    }
    ...
    function __SwapStakingContract_init(
        ...
        require(
            _maxStakingAmount > 0,
            "[Validation] _maxStakingAmount has to be larger than 0"
        );
    ...
    function setTokenAddress(
        address _token
    ) external onlyContract(_token) whenPaused {
        require(
            hasRole(OWNER_ROLE, _msgSender()),
            "[Validation] The caller must have owner role to set token address"
        );
        require(
            _token != address(0),
            "[Validation] Invalid swap token address"
    ...
    function deposit(uint256 amount) external nonReentrant whenNotPaused {
        ...
        uint256 resultedStakedAmount = currentTotalStake.add(
            newPrincipal.sub(oldPrincipal)
        );
        require(
            resultedStakedAmount <= maxStakingAmount,
            "[Deposit] Your deposit would exceed the current staking limit"
    ...
    function executeWithdrawal() external nonReentrant whenNotPaused {
        ...
        if (stakeDeposit.amount > amount) {
            _stakeDeposits[msg.sender].amount = _stakeDeposits[msg.sender]
                .amount
                .sub(amount);
    ...
    function withdrawRewards() external nonReentrant whenNotPaused {
        ...
        require(
            stakeDeposit.endDate == 0,
            "[Rewards Withdrawal] You already initiated the full withdrawal"
        );
        ...
        require(
            reward > 0,
            "[Rewards Withdrawal] The reward amount has to be larger than 0"
        );
    ...
    function _computeReward(
        ...
        if (stakeDeposit.endDate == 0) {
            rewardsPoints = totalRewardPoints.sub(
                stakeDeposit.entryRewardPoints
            );
        } else {
            //withdrawal is initiated
            rewardsPoints = stakeDeposit.exitRewardPoints.sub(
                stakeDeposit.entryRewardPoints
            );
        }
    ...
    function _distributeRewards() private whenNotPaused {
        ...
        require(
            currentTotalStake > 0,
            "[Validation] not enough total stake accumulated"
        );
        uint256 rewardPoolBalance = token.balanceOf(rewardsAddress);
        require(
            rewardPoolBalance > 0,
            "[Validation] not enough rewards accumulated"
        );

        uint256 newlyAdded = rewardPoolBalance.add(rewardsWithdrawn).sub(
            rewardsDistributed
        );
